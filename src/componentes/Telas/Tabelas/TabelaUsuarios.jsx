...
export default function TabelaUsuarios(props) {
            ...
            excluirUsuario(usuario).then((resposta) => {
                if (resposta.status) {
                    props.setListaDeUsuarios(
                        props.listaDeUsuarios.filter(
                            (item) => item.codigo !== usuario.codigo
                        )
                    );
                } else {
                    window.alert("Não foi possível excluir o usuário: " + resposta.mensagem);
                }
            });
        }
    }
    ...
    return (
        <Container>
                    ...
                    {props.listaDeUsuarios?.map((usuario) => (
                        <tr key={usuario.codigo}>
                            ...
                            <td>
                                ...
                                <Button
                                    onClick={() => excluirUsuarioFrontEnd(usuario)}
                                    variant="danger"
            ...
            <p>Quantidade de usuários cadastrados: {props.listaDeUsuarios?.length || 0}</p>
        </Container>
    );
}
